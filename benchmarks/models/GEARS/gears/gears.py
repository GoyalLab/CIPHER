...
def _make_subset_loader(loader, n_cells_per_pert):
    """Return a loader capped at n_cells_per_pert samples per perturbation."""
    from collections import defaultdict
    from torch.utils.data import Subset
    from torch_geometric.data import DataLoader as PyGDataLoader

    pert_to_idx = defaultdict(list)
    for i, data in enumerate(loader.dataset):
        pert_to_idx[tuple(data.pert)].append(i)

    subset_indices = []
    for indices in pert_to_idx.values():
        subset_indices.extend(indices[:n_cells_per_pert])
...
class GEARS:
    ...
    def __init__(self, pert_data, 
                 model_save_folder, # IRISH Custom variable
                 device = 'cuda',
    ...
    def train(self, epochs = 20,
        ...
        weight_decay: float
            weight decay
        n_eval_cells: int or None
            number of cells per perturbation to use for end-of-epoch evaluation.
            None (default) uses the full dataset.
        force_retrain: bool
            if True, ignore existing checkpoints and train from scratch. Default False.
        ...
        # Irish: Resume training from a checkpoint if it exists
        resumed_epoch_count = 0
        resumed_best_model_state = None
        resumed_min_val = None
        if not force_retrain:
            for file in sorted(glob.glob(self.model_save_folder + "/*"), key=os.path.getmtime, reverse=True):
                file_name = file.split("/")[-1]
                if (file_name.startswith("epoch_") is True) and (file_name.endswith("checkpoint.pt") is True):
                    checkpoint = torch.load(file)
                    if isinstance(checkpoint, dict) and 'model' in checkpoint:
                        self.model.load_state_dict(checkpoint['model'])
                        optimizer.load_state_dict(checkpoint['optimizer'])
                        # scheduler state intentionally not restored — allows a fresh
                        # cosine cycle when resuming with a different epoch count
                        resumed_best_model_state = checkpoint.get('best_model')
                        resumed_min_val = checkpoint.get('min_val')
                    else:
                        self.model.load_state_dict(checkpoint)
                    resumed_epoch_count = int(file_name.split("_")[1]) + 1
                    break

        # best_model/min_val are derived from self.model *after* the resume load above
        # (and from the saved best-model snapshot, if any), never from the pre-resume
        # weights. Otherwise a run that finishes all its epochs via checkpoints but
        # crashes before/during testing would resume with zero epochs left to train, and
        # silently score with an untrained model instead of the actual trained one.
        best_model = deepcopy(self.model)
        if resumed_best_model_state is not None:
            best_model.load_state_dict(resumed_best_model_state)
        min_val = resumed_min_val if resumed_min_val is not None else np.inf

        _loss_file = os.path.join(self.model_save_folder, 'epoch_losses.npy')
        epoch_losses = list(np.load(_loss_file)) if (os.path.exists(_loss_file) and not force_retrain) else []
        _de_mse_file = os.path.join(self.model_save_folder, 'epoch_de_mse.npy')
        de_mse_list = list(np.load(_de_mse_file)) if (os.path.exists(_de_mse_file) and not force_retrain) else []
        _val_loss_file = os.path.join(self.model_save_folder, 'epoch_val_losses.npy')
        val_losses = list(np.load(_val_loss_file)) if (os.path.exists(_val_loss_file) and not force_retrain) else []
        _val_de_mse_file = os.path.join(self.model_save_folder, 'epoch_val_de_mse.npy')
        val_de_mse_list = list(np.load(_val_de_mse_file)) if (os.path.exists(_val_de_mse_file) and not force_retrain) else []

        for epoch in range(epochs - resumed_epoch_count):
            self.model.train()
            step_losses = []

            import sys
            from tqdm import tqdm
            pbar = tqdm(train_loader, desc=f'Epoch {epoch + resumed_epoch_count + 1}', leave=True,
                        disable=not sys.stderr.isatty(), file=sys.stderr)
            ...
            np.save(_loss_file, epoch_losses)

            print_sys("Evaluating model...")
            _eval_train_loader = _make_subset_loader(train_loader, n_eval_cells) if n_eval_cells is not None else train_loader
            _eval_val_loader   = _make_subset_loader(val_loader,   n_eval_cells) if n_eval_cells is not None else val_loader
            train_res = evaluate(_eval_train_loader, self.model, self.config['uncertainty'], self.device)
            val_res   = evaluate(_eval_val_loader,   self.model, self.config['uncertainty'], self.device)
            train_metrics, _ = compute_metrics(train_res)
            val_metrics,   _ = compute_metrics(val_res)

            de_mse_list.append(float(train_metrics['mse_de']))
            val_losses.append(float(val_metrics['mse']))
            val_de_mse_list.append(float(val_metrics['mse_de']))
            np.save(_de_mse_file, de_mse_list)
            np.save(_val_loss_file, val_losses)
            np.save(_val_de_mse_file, val_de_mse_list)
            import matplotlib
            matplotlib.use('Agg')
            import matplotlib.pyplot as plt
            epochs_x = range(1, len(epoch_losses) + 1)
            fig, ax1 = plt.subplots()
            ax1.plot(epochs_x, epoch_losses, color='tab:blue', label='Train Loss')
            ax1.set_xlabel('Epoch')
            ax1.set_ylabel('Mean Training Loss', color='tab:blue')
            ax1.tick_params(axis='y', labelcolor='tab:blue')
            ax2 = ax1.twinx()
            ax2.plot(range(1, len(de_mse_list) + 1),     de_mse_list,     color='tab:orange', label='Train DE MSE')
            ax2.plot(range(1, len(val_de_mse_list) + 1), val_de_mse_list, color='tab:orange', label='Val DE MSE',  linestyle='dashed')
            ax2.set_ylabel('Top 20 DE MSE', color='tab:orange')
            ...
            fig.tight_layout()
            plt.savefig(os.path.join(self.model_save_folder, 'loss_curve.png'))
            plt.close()
            ...
            torch.save({
                ...
                'min_val': min_val,
            }, os.path.join(self.model_save_folder, "epoch_" + str(epoch + resumed_epoch_count) + "_checkpoint.pt"))
        ...
        _eval_test_loader = _make_subset_loader(test_loader, n_eval_cells) if n_eval_cells is not None else test_loader
        test_res = evaluate(_eval_test_loader, self.best_model,
