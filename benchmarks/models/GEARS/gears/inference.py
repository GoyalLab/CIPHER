...
def evaluate(loader, model, uncertainty, device):
    ...
    import sys
    for itr, batch in enumerate(tqdm(loader, desc='Evaluating', leave=False,
...
def non_zero_analysis(adata, test_res):
    ...
    for pert in np.unique(test_res['pert_cat']):
        pert_metric[pert] = {}
        full_id = pert2pert_full_id.get(pert)
        if full_id not in adata.uns['top_non_zero_de_20']:
            continue

        pert_idx = np.where(test_res['pert_cat'] == pert)[0]
        de_idx = [geneid2idx[i] for i in adata.uns['top_non_zero_de_20'][full_id]]
...
def non_dropout_analysis(adata, test_res):
    ...
    for pert in np.unique(test_res['pert_cat']):
        pert_metric[pert] = {}
        full_id = pert2pert_full_id.get(pert)
        if (full_id not in adata.uns['top_non_dropout_de_20'] or
                full_id not in adata.uns['non_zeros_gene_idx'] or
                full_id not in adata.uns['non_dropout_gene_idx']):
            continue

        pert_idx = np.where(test_res['pert_cat'] == pert)[0]
        de_idx = [geneid2idx[i] for i in adata.uns['top_non_dropout_de_20'][full_id]]
        non_zero_idx = adata.uns['non_zeros_gene_idx'][full_id]
        non_dropout_gene_idx = adata.uns['non_dropout_gene_idx'][full_id]
...
def deeper_analysis(adata, test_res, de_column_prefix = 'rank_genes_groups_cov', most_variable_genes = None):
    ...
    for pert in np.unique(test_res['pert_cat']):
        pert_metric[pert] = {}
        full_id = pert2pert_full_id.get(pert)
        if full_id not in adata.uns['rank_genes_groups_cov_all']:
            continue
        de_idx = [geneid2idx[i] for i in adata.uns['rank_genes_groups_cov_all'][full_id][:20]]
        de_idx_200 = [geneid2idx[i] for i in adata.uns['rank_genes_groups_cov_all'][full_id][:200]]
        de_idx_100 = [geneid2idx[i] for i in adata.uns['rank_genes_groups_cov_all'][full_id][:100]]
        de_idx_50 = [geneid2idx[i] for i in adata.uns['rank_genes_groups_cov_all'][full_id][:50]]
