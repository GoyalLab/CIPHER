...
class GeneSimNetwork():
    ...
    def __init__(self, edge_list, gene_list, node_map):
        ...
        valid_edges = [e for e in self.G.edges if e[0] in node_map and e[1] in node_map]
        edge_index_ = [(node_map[e[0]], node_map[e[1]]) for e in valid_edges]
        self.edge_index = torch.tensor(edge_index_, dtype=torch.long).T

        edge_attr = nx.get_edge_attributes(self.G, 'importance')
        importance = np.array([edge_attr[e] for e in valid_edges])
...
def make_GO(data_path, pert_list, data_name, num_workers=25, save=True):
    ...
    # Irish: was a hardcoded relative path ('./data/...'), which resolved against
    # whatever the process's cwd happened to be (e.g. the SLURM --chdir dir) instead
    # of the dataset's own data_path, silently scattering ~350MB CSVs outside the
    # configured results folder. Derive it from data_path like every other cache file.
