...
class PertData:
    ...
    def new_data_process(self, dataset_name,
        ...
        # Irish: No need to create the dataset file if it already exists
        if os.path.exists(dataset_fname) is True:
            print("Found pyg file at " + dataset_fname)
            print("The pyg file is " + str(round(os.path.getsize(dataset_fname) / (1024 * 1024 * 1024), 2)) + " GB, so get comfy...")
            
            with open(dataset_fname, "rb") as file:
                self.dataset_processed = pickle.load(file)
        else:
            if not os.path.exists(pyg_path):
                os.mkdir(pyg_path)
            print_sys(dataset_fname)
            print_sys("Creating pyg object for each cell in the data...")
            self.create_dataset_file()
            print_sys("Saving new dataset pyg object at " + dataset_fname) 
            pickle.dump(self.dataset_processed, open(dataset_fname, "wb"))    
    ...
    def create_cell_graph_dataset(self, split_adata, pert_category,
        ...
        if pert_category != 'ctrl':
            ...
            if de:
                fallback = list(adata_.var_names[:num_de_genes])
                de_idx = np.where(adata_.var_names.isin(
                    np.array(de_genes.get(pert_de_category, fallback)[:num_de_genes])))[0]
            else:
