...
results = None
Sp = p_lr = p_mf = p_lfc = None
Sd = md_ = d_lr = d_mf = d_lfc = None
...
def run_identify(S, name, h_mode="sigma_count"):
    ...
    return m, r_lr, r_mf, r_lfc


def _plot_identify(r_lr, r_mf, r_lfc, title):
    ks = list(range(1, 11)); fig, ax = plt.subplots(1, 3, figsize=(15, 4))
    for r, lab in [(r_lr, "CIPHER (linear response)"), (r_mf, "mean-field"), (r_lfc, "LFC cosine")]:
        ax[0].plot(ks, [r.topk_acc[k] for k in ks], marker="o", label=lab)
        if r.roc:
            ax[1].plot(*r.roc, label=lab)
        if r.prc:
            pr, rc = r.prc; ax[2].plot(rc, pr, label=lab)
    ax[1].plot([0, 1], [0, 1], "k--", lw=1)
    ax[0].set(xlabel="k", ylabel="top-k accuracy", title=f"{title}: top-k")
    ax[1].set(xlabel="false positive rate", ylabel="true positive rate", title="margin ROC")
    ax[2].set(xlabel="recall", ylabel="precision", title="margin PR")
    for a in ax:
        a.legend(fontsize=8)
    fig.tight_layout()
    return fig
        ...
        for p in paths:
            try:
                res = posterior_inverse_prediction(p, normalization=NORM, method="posterior",
                                                   progress=False, expression_threshold=0.1, min_samples=100)
...
def run_crispri():
    ...
    _mp, p_lr, p_mf, p_lfc = run_identify(Sp, ds_p.name, h_mode="sigma_count")
...
def plot_crispri():
    ...
    fig = _plot_identify(p_lr, p_mf, p_lfc, "CRISPRi perturbation ID")
...
def run_sciplex():
    ...
    md_, d_lr, d_mf, d_lfc = run_identify(Sd, ds_d.name, h_mode="within_cov")
...
def plot_sciplex(query="Fulvestrant"):
    ...
    fig = _plot_identify(d_lr, d_mf, d_lfc, "sci-Plex drug ID")
    ...
    colors = ["crimson" if Sd["labels"][i] == q else "0.6" for i in order]
    fig, ax = plt.subplots(figsize=(6, 5)); ypos = np.arange(len(order))[::-1]
    ax.barh(ypos, ll[order], color=colors); ax.set_yticks(ypos); ax.set_yticklabels(lab, fontsize=8)
    ax.set(xlabel="log-likelihood (linear response)", title=f"J   candidate drugs for query = {q}")
    fig.tight_layout(); fig.savefig(os.path.join(OUTDIR, "fig5_candidate_ll.svg")); plt.show()


def _margin_scores(r):
    c = (r.rank <= 1).astype(int); m = np.asarray(r.margin, float); fin = np.isfinite(m)
...
def panels_hi_composite():
    """H/I values -- composite margin AUROC/AUPRC for CIPHER vs mean-field vs LFC (CRISPR + sci-Plex)."""
    rows = []
    for grp, (rl, rm, rf) in {"CRISPRi/a": (p_lr, p_mf, p_lfc), "sci-Plex": (d_lr, d_mf, d_lfc)}.items():
        for meth, r in [("linear response", rl), ("linear response (mean-field)", rm), ("log fold change", rf)]:
            au, ap = _margin_scores(r); rows.append(dict(group=grp, method=meth, AUROC=au, AUPRC=ap))
